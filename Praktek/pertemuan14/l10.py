...
class MainForm (QMainWindow):
    ...
    def setupUi(self):
        ...
        # membuat menu File dan menempatkannya ke dalam menubar fileMenu = menubar.addMenu('&File')
        ...
        fileOpenAction.setShortcut('Ctrl+0')
        ...
        # membuat menu Edit dan menempatkannya ke dalam menubar editMenu = menubar.addMenu('&Edit')
        ...
        editPasteAction.setStatusTip(
            'Tempel teks (yang telah dipotong/disalin)')
    ...
    def confirmation(self):
        if self.textEdit.document().isModified():
            response = QMessageBox.question(
                self, 'Konfirmasi', 'Teks telah dimodifikasi. Simpan?')
    ...
    def fileSaveAsActionTriggered(self):
        fileName = QFileDialog.getSaveFileName(self,
                                               'Simpan file', os.curdir, 'File Teks (*.txt)', '*.txt')
        if not fileName[0]:
            return
        self.currentFileName = fileName[0]
        self.setWindowTitle(PROGRAM_NAME + self.writeToFile()
                            + self.currentFileName)
    ...
    def formatFontActionTriggered(self):
        fontTuple = QFontDialog.getFont(QFont('Sans Serif', 11),
                                        self, 'Pilih font')
        if fontTuple[0]:
